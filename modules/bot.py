...
piic = "https://telegra.ph//file/26e162bf339e4807c1286.jpg"

buttons = [
    [
        Button.url(get_string("bot_4"), "t.me/GokilSupport"),
    ]
]
...
absen = [
    "**Hadir Sayang** 😳",
    "**Hadir Bro Lucifer** 😁",
    "**Maaf ka habis nemenin ka Lucifer** 🥺",
    "**Maaf ka habis disuruh Tuan Lucifer** 🥺🙏🏻",
    "**Hadir Lucifer Sayang** 😘",
    "**Hadir Lucifer Akuuuuhhh** ☺️",
    "**Hadir Lucifer brother Aku** 🥰",
    "**Sokap bet lu**",
    "**Apasi Bawel** 🥰",
]
...
@register(incoming=True, from_users=DEVS, pattern=r"^Lucifer$")
async def abel(abel):
    await abel.reply("**Lucifer Punya Nya Abel**🤩")


@register(incoming=True, from_users=DEVS, pattern=r"^Abel$")
async def abel(abel):
    await abel.reply("**Abel Punya Nya Lucifer**🤩")
...
async def _(event):
    ...
    if opt == "heroku":
        ...
        await event.reply("**Luci Logs.**", file=file)
    elif opt == "open":
...
async def _(e):
    ...
    if m:
        ...
        await xx.edit(
            f'<code>Your BOT is </code><strong>up-to-date</strong><code> with </code><strong><a href="https://github.com/jonesroot/Lucifer-UserBot/tree/{branch}">[{branch}]</a></strong>',
            parse_mode="html",
            link_preview=False,
        )
